...
bool CheckAddress(char* ip_)//проверка допустимого диапазона октета
{
...
unsigned long CharToLong(char* ip_) //преобразование строки IP-адреса (16 символов) в десятично - точечной нотации
{
...
int main()
{
	...
	subnet = ip & mask; //получение подсети
	host = ip & ~mask; //получение хоста
	broadcast = ip & mask | ~mask; //получение бродкаста
